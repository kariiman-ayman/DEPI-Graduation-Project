...
export default function StudentCourses() {
  ...
  function onTabChange(next: string) {
    ...
    setActiveTab(value);
    setSearchParams(value === "available" ? { tab: "available" } : {}, {
      replace: true,
    });
  }
                ...
                {enrolledCourses.map((course) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Badge
                        className={`shrink-0 ${gradeColor(course.grade?.letterGrade ?? null)}`}
                      >
                    ...
                    <Button
                      ...
                      className="w-full"
                      onClick={() =>
                        navigate(`/lectures?course=${course.courseId}`)
                      }
                    >
            ...
            availableCourses.length > 0 && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {availableCourses.map((course) => {
                  ...
                  const yearBlocked =
                    course.minYear !== null &&
                    (studentYear === null || studentYear < course.minYear);
                  ...
                  if (alreadyEnrolled) buttonLabel = "Already Enrolled";
                    ...
                    buttonLabel = `Year ${course.minYear}+ Required`;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        {course.minYear !== null && (
                          <p
                            className={`flex items-center gap-1 text-xs ${yearBlocked ? "text-amber-600 dark:text-amber-400" : "text-gray-400 dark:text-gray-500"}`}
                          >
                            <Layers className="w-3 h-3" />
                            Requires Year {course.minYear} or above
                            {yearBlocked &&
                              studentYear !== null &&
                              ` (you are Year ${studentYear})`}
                          </p>
                        )}
                      </div>
