...
import {
  ShieldCheck, Mail, Calendar, User, Lock, CheckCircle,
  AlertCircle, Send, Users, MailCheck, Pencil, X,
} from "lucide-react";
...
export default function AdminProfile() {
  ...
  const acceptRate = profile.stats.totalInvites > 0
    ? Math.round((profile.stats.acceptedInvites / profile.stats.totalInvites) * 100)
    : 0;
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <StatCard icon={Send} label="Invitations Sent" value={String(profile.stats.totalInvites)} color="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400" />
        <StatCard icon={MailCheck} label="Accepted" value={`${profile.stats.acceptedInvites} (${acceptRate}%)`} color="bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400" />
        <StatCard icon={Users} label="Total Users" value={String(profile.stats.totalUsers)} color="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 space-y-5 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            {!editName && (
              <button onClick={startEdit} className="flex items-center gap-1 text-xs text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 transition-colors">
            ...
            {editName ? (
              <div className="mt-1 space-y-2">
                ...
                <div className="flex gap-2">
                  <Button size="sm" className="bg-red-600 hover:bg-red-700" disabled={saving} onClick={handleSaveName}>
                    {saving ? "Saving…" : "Save"}
                  </Button>
              ...
              <div className="mt-1 flex items-center gap-2">
                <p className="text-sm text-gray-700 dark:text-gray-200 bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2 flex-1">{displayName}</p>
                {nameSuccess && (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 shadow-sm">
          ...
          <form onSubmit={handleChangePwd} className="space-y-4">
            ...
            <Button type="submit" disabled={changingPwd || !currentPwd || !newPwd || !confirmPwd} className="w-full bg-red-600 hover:bg-red-700">
