...
import {
  Users, Mail, Calendar, User, Lock, CheckCircle,
  AlertCircle, BookOpen, GraduationCap, Pencil, X, BookMarked,
} from "lucide-react";
...
function getInitials(name?: string | null) {
  if (!name) return "I";
  return name.split(" ").filter(Boolean).slice(0, 2).map((p) => p[0]!.toUpperCase()).join("") || "I";
}
...
export default function InstructorProfile() {
  ...
  const handleSaveInfo = async () => {
    setInfoError("");
    try {
      await saveProfile({ name: nameVal, title: titleVal || undefined, specialization: specVal || undefined });
      setInfoSuccess(true);
      setEditMode(false);
  ...
  const displayName = profile.title ? `${profile.title} ${profile.name}` : profile.name;
  ...
  return (
    <div className="max-w-4xl space-y-6">
            ...
            <div className="flex items-end gap-4">
              ...
              <div className="pb-1">
                ...
                {profile.specialization && (
                  <p className="text-sm text-gray-500 dark:text-gray-400">{profile.specialization}</p>
                )}
      ...
      <div className="grid grid-cols-2 gap-4">
        <StatCard icon={BookOpen} label="Courses Taught" value={String(profile.stats.totalCourses)} color="bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400" />
        <StatCard icon={GraduationCap} label="Students Enrolled" value={String(profile.stats.totalStudents)} color="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 space-y-5 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            {!editMode && (
              <button onClick={startEdit} className="flex items-center gap-1 text-xs text-purple-600 dark:text-purple-400 hover:text-purple-800 dark:hover:text-purple-300 transition-colors">
          ...
          {editMode ? (
            <>
              ...
              <div>
                ...
                <Input id="name" className="mt-1" value={nameVal} onChange={(e) => setNameVal(e.target.value)} />
              </div>
              <div>
                <Label htmlFor="spec" className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Specialization</Label>
                <div className="relative mt-1">
                  <BookMarked className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                  <Input id="spec" className="pl-9" placeholder="e.g. Computer Science" value={specVal} onChange={(e) => setSpecVal(e.target.value)} />
                </div>
              </div>
              {infoError && (
              ...
              <div className="flex gap-2">
                <Button size="sm" className="bg-purple-600 hover:bg-purple-700" disabled={saving} onClick={handleSaveInfo}>
            ...
            <>
              <div>
                ...
                <p className="mt-1 text-sm text-gray-700 dark:text-gray-200 bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2">
                  {profile.title || <span className="text-gray-400 italic">Not set</span>}
                </p>
              </div>
              <div>
              ...
              <div>
                <Label className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Specialization</Label>
                <p className="mt-1 text-sm text-gray-700 dark:text-gray-200 bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 shadow-sm">
          ...
          <form onSubmit={handleChangePwd} className="space-y-4">
            ...
            <Button type="submit" disabled={changingPwd || !currentPwd || !newPwd || !confirmPwd} className="w-full bg-purple-600 hover:bg-purple-700">
