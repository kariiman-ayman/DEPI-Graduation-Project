...
export const getLectures = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const progressSnap = await db.collection("lectureProgress")
      .where("studentId", "==", studentId)
      .get();
    const progressMap = new Map(progressSnap.docs.map((d) => [d.data().lectureId as string, d.data()]));

    // 5. Fetch unique instructors
    const instructorIds = [...new Set(allLectureDocs.map((d) => d.data().instructorId as string))];
    const instructorDocs = await Promise.all(instructorIds.map((id) => db.collection("users").doc(id).get()));
...
export const saveProgress = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { progress, watchedSeconds } = req.body as { progress: number; watchedSeconds: number };
    ...
    const existingSnap = await db.collection("lectureProgress")
      .where("studentId", "==", studentId)
    ...
    if (existingSnap.empty) {
      await db.collection("lectureProgress").add({ ...payload, createdAt: new Date() });
    } else {
