...
export const getMyGrades = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (enrollmentsSnap.empty) {
      return res.json({ grades: [], gpa: 0, totalCredits: 0, gradedCredits: 0 });
    }

    const courseIds = enrollmentsSnap.docs.map((d) => d.data().courseId as string);
    ...
    gradesSnap.docs.forEach((d) => gradeMap.set(d.data().courseId as string, d.data()));
    ...
    const gpa = gradedCredits > 0
      ? +( weightedPoints / gradedCredits).toFixed(2)
      : 0;
