...
export const getProfile = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [userDoc, invitesSnap, usersSnap] = await Promise.all([
    ...
    const acceptedInvites = invites.filter((i) => i.used).length;
...
export const updateProfile = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await db.collection("users").doc(uid).update({ name: name.trim() });
