...
export const getCourseGrades = async (req: AuthRequest, res: Response) => {
  try {
    ...
    gradesSnap.docs.forEach((d) =>
      gradeMap.set(d.data().studentId as string, d.data()),
    );

    // Collect unique student IDs from enrollments
    const studentIds = enrollmentsSnap.docs.map(
      (d) => d.data().studentId as string,
    );
...
export const upsertGrade = async (req: AuthRequest, res: Response) => {
  try {
    const instructorId = req.user?.uid;
    const { studentId, courseId, midterm, assignments, project, final } =
      req.body as {
        studentId: string;
        courseId: string;
        midterm: number;
    ...
    for (const [key, val] of Object.entries(scores)) {
      if (typeof val !== "number" || val < 0 || val > 100) {
        return res
          .status(400)
          .json({ message: `${key} must be a number between 0 and 100` });
      }
    }
    ...
    if (existingSnap.empty) {
      const docRef = await db
        .collection("grades")
        .add({ ...gradePayload, createdAt: now });
      ...
      savedGrade = {
        id: docRef.id,
        ...gradePayload,
        createdAt: existingSnap.docs[0].data().createdAt,
      };
    }
